...
from datasets.textual_desc import get_class_labels_from_split
...
def get_dataset(args):
    ...
    class_labels = get_class_labels_from_split(dataset_name=args.dataset)
    
    return train_loader, val_loader, test_loader, n_cls, class_labels

def main(args):
    ensure_path(args.save_path)

    train_loader, val_loader, test_loader, n_cls, class_labels = get_dataset(args)
    ...
    for epoch in range(start_epoch, args.max_epoch + 1):
        ...
        tl, ta = train(args, model, train_loader, optimizer, teacher, criterion_kd, class_labels)
        lr_scheduler.step()
        vl, va, aa, bb = validate(args, model, val_loader)
        # Additional validation on test dataset
        test_loss, test_acc, test_acc_mean, test_acc_ci = validate(args, model, test_loader, class_labels)
...
def train(args, model, train_loader, optimizer, teacher, kd_loss, class_labels):
    model.train()
    Omodel.eval()
    teacher.eval()
    ...
    for i, batch in enumerate(train_loader, 1):
        ...
        text_features = text_features[:, :120] 
        ...
        loss = ((1.0 - args.alpha) * F.cross_entropy(logits, label)) + (args.alpha * kdloss)
...
def validate(args, model, val_loader, class_labels):
    model.eval()
    Omodel.eval()
