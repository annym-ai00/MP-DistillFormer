...
class TextProjection(nn.Module):
    def __init__(self, text_dim=512, visual_dim=120):
        super().__init__()
...
def main(args):
    ...
    text_projection = TextProjection().cuda()

    optimizer = torch.optim.Adam(list(model.parameters()) + list(text_projection.parameters()), lr=args.lr, weight_decay=args.wd)
    ...
    for epoch in range(start_epoch, args.max_epoch + 1):
        ...
        tl, ta = train(args, model, train_loader, optimizer, teacher, criterion_kd, text_projection, train_labels)
        lr_scheduler.step()
        vl, va, aa, bb = validate(args, model, val_loader, text_projection, valid_labels)
        # Additional validation on test dataset
        test_loss, test_acc, test_acc_mean, test_acc_ci = validate(args, model, test_loader, text_projection, test_labels)
...
def train(args, model, train_loader, optimizer, teacher, kd_loss, text_projection, class_labels):
    model.train()
    Omodel.eval()
    teacher.eval()
    ...
    for i, batch in enumerate(train_loader, 1):
        ...
        projected_text_features = text_projection(text_features.float())
        ...
        proto = ((1 - args.delta) * proto_image_features) + (args.delta * projected_text_features[:args.train_way, :])
...
def validate(args, model, val_loader, text_projection, class_labels):
    model.eval()
    Omodel.eval()
    ...
    with torch.no_grad():
        ...
        for i, batch in enumerate(val_loader, 1):
            ...
            proto = ((1 - args.delta) * proto_image_features) + (args.delta * projected_text_features[:args.test_way, :])
            query = query_image_features  
